...
# htslib
# --------------------------------------------------------------
rm -rf htslib-1.15.1 htslib-1.15.1.tar.bz2 >& /dev/null

wget https://github.com/samtools/htslib/releases/download/1.15.1/htslib-1.15.1.tar.bz2
tar -xf htslib-1.15.1.tar.bz2
cd htslib-1.15.1
./configure \
--prefix=/opt/entorepo/lib/htslib/1.15.1
make -j12
rm -rf /opt/entorepo/lib/htslib/1.15.1 >& /dev/null
make install

cd /opt/entorepo/lib/htslib
ln -sfT 1.15.1 current
...
cd $CURRDIR
rm -rf htslib-1.15.1 htslib-1.15.1.tar.bz2
# --------------------------------------------------------------

# samtools
# --------------------------------------------------------------
rm -rf samtools-1.15.1 samtools-1.15.1.tar.bz2 >& /dev/null
...
./configure \
--prefix=/opt/entorepo/apps/samtools/1.15.1 \
--with-htslib=/opt/entorepo/lib/htslib/1.15.1
make
rm -rf /opt/entorepo/apps/samtools/1.15.1 >& /dev/null
make install
...
# bcftools
# --------------------------------------------------------------
rm -rf bcftools-1.15.1 bcftools-1.15.1.tar.bz2 >& /dev/null

wget https://github.com/samtools/bcftools/releases/download/1.15.1/bcftools-1.15.1.tar.bz2
tar -xf bcftools-1.15.1.tar.bz2
cd bcftools-1.15.1
./configure \
--prefix=/opt/entorepo/apps/bcftools/1.15.1 \
--with-htslib=/opt/entorepo/lib/htslib/1.15.1 
make
rm -rf /opt/entorepo/apps/bcftools/1.15.1 >& /dev/null
make install
...
perl -pi -e 's/\#\!\/usr\/bin\/perl \-w/\#\!\/usr\/bin\/env perl/g' /opt/entorepo/apps/bcftools/1.15.1/bin/vcfutils.pl
perl -pi -e 's/\#\!\/usr\/bin\/python/\#\!\/usr\/bin\/env python/g' /opt/entorepo/apps/bcftools/1.15.1/bin/plot-roh.py
